...
print(f"[✔] successfully loaded {len(missions_database.objects)} missions and {len(telescope_database.objects)} telescopes")

# Update telescope availability
print(telescope_availability)
...
if len(schedule.missions) > 0:
    for mission in schedule.missions:
        print(addMission(mission, "Basic", missions_database))

if len(schedule.advanced_missions) > 0:
    for mission in schedule.advanced_missions:
        print(addMission(mission, "Advanced", missions_database))
